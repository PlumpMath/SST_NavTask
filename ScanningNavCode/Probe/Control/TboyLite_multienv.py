...
## added 4/6/16
        ...
        # Read list of shock subs from external text file.
        #subj_list = []
        slist       = open('../shocklist.txt', 'r')
        #for line in slist:
            #subj_list.append(line.strip())
        sublist = slist.read()
        subj_list = [line.strip() for line in sublist.split(',')]
        #subj_list = slist.read().split(',')
        slist.close()
        print subj_list
##
...
##        subj_list = ['14', '15', '16', '100']
        if options.subjId in subj_list: #here we override default of fixation cross for shock warn
            state['fixation'] = 0
            print options.subjId
            # reset fixation state to shockwarn
        ...
        if state['currentDelivery'] >= 0 and \
           ...
           store == state['stores'][state['deliveries'][state['currentDelivery']]]:
            ...
            Instructions("deliveryMade", 
                         config['deliveryMadeText'] % self.storeName(store)).display()
